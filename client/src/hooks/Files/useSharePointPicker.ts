...
export default function useSharePointPicker({
  ...
  const cleanup = useCallback(() => {
    ...
    // 安全地移除 iframe，而不是清空整个 innerHTML
    // 这样可以避免与 React 的 DOM 操作冲突
    if (iframeRef.current && containerNode) {
      try {
        // 检查 iframe 是否仍然是 containerNode 的子节点
        if (containerNode.contains(iframeRef.current)) {
          containerNode.removeChild(iframeRef.current);
        }
      } catch (error) {
        // 节点可能已经被 React 移除，这是正常的
        console.warn('Error removing iframe:', error);
      }
      iframeRef.current = null;
    }
    channelIdRef.current = '';
  }, [containerNode, portMessageHandler]);
