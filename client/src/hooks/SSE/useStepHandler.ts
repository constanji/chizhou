...
export default function useStepHandler({
  ...
  const updateContent = (
    ...
    if (
      ...
      let args: string | Record<string, unknown> | null | undefined;
      
      if (finalUpdate || typeof existingToolCall?.args === 'object' || typeof toolCallArgs === 'object') {
        // Use the provided args directly for final updates or when args are objects
        args = contentPart.tool_call.args;
      } else {
        // For streaming string args, concatenate them
        // Ignore null values during streaming (e.g., ModelScope sends null as final chunk)
        if (toolCallArgs == null && existingToolCall?.args != null && typeof existingToolCall.args === 'string') {
          // Preserve existing args when delta is null
