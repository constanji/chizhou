  ...
  return useQuery<t.TConversation>(
    ...
    async () => {
      ...
      try {
        ...
        // 处理404错误：对话不存在时返回null而不是抛出错误
        if (error?.response?.status === 404) {
          logger.warn('conversation', `Conversation ${id} not found (404), returning null`);
          // 返回null而不是抛出错误，这样查询不会处于错误状态
          return null as any;
        }
        throw error;
      }
    },
    {
      ...
      // 404时不应该被视为错误状态，使用useErrorBoundary: false
