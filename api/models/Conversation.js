...
module.exports = {
  ...
  saveConvo: async (req, { conversationId, newConversationId, ...convo }, metadata) => {
    try {
      ...
      logger.info(`[saveConvo] Attempting to save conversation: ${finalConversationId}, user: ${req.user?.id}`);

      const messages = await getMessages({ conversationId }, '_id');
      logger.debug(`[saveConvo] Found ${messages?.length || 0} messages for conversation ${finalConversationId}`);
      ...
      const conversation = await Conversation.findOneAndUpdate(
        { conversationId: finalConversationId, user: req.user.id },
        updateOperation,
        {
          new: true,
          upsert: true,
      ...
      if (!conversation) {
        logger.error(`[saveConvo] Failed to save conversation ${finalConversationId} - findOneAndUpdate returned null`);
        throw new Error(`Failed to save conversation ${finalConversationId}`);
      }

      logger.info(`[saveConvo] Successfully saved conversation: ${finalConversationId}`);
      return conversation.toObject();
    } catch (error) {
      logger.error('[saveConvo] Error saving conversation', error);
      logger.error(`[saveConvo] ConversationId: ${conversationId || newConversationId}, User: ${req.user?.id}`);
      if (metadata && metadata?.context) {
        logger.info(`[saveConvo] ${metadata.context}`);
      }
      // 不要返回错误对象，而是抛出错误让调用方处理
      throw error;
    }
  },
