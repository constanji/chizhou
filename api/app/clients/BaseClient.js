...
class BaseClient {
  ...
  getTokenCountForResponse(responseMessage) {
    logger.debug('[BaseClient] `recordTokenUsage` 未实现。', responseMessage);
  }
  ...
  async recordTokenUsage({ model, balance, promptTokens, completionTokens }) {
    logger.debug('[BaseClient] `recordTokenUsage` 未实现。', {
      model,
      balance,
      promptTokens,
      completionTokens,
  ...
  async handleTokenCountMap(tokenCountMap) {
    ...
    for (let i = 0; i < this.currentMessages.length; i++) {
      ...
      if (messageId === tokenCountMap.summaryMessage?.messageId) {
        logger.debug(`[BaseClient] 正在向 ${messageId} 添加摘要属性。`);
      ...
      if (message.tokenCount && !update.summaryTokenCount) {
        logger.debug(`[BaseClient] 跳过 ${messageId}: 已有 token 计数。`);
        continue;
      }
  ...
  async handleContextStrategy({
    ...
    _instructions && logger.debug('[BaseClient] 指令 token 计数: ' + tokenCount);
    if (tokenCount && tokenCount > this.maxContextTokens) {
      const info = `${tokenCount} / ${this.maxContextTokens}`;
      const errorMessage = `{ "type": "${ErrorTypes.INPUT_LENGTH}", "info": "${info}" }`;
      logger.warn(`指令 token 计数超过最大 token 计数 (${info})。`);
    ...
    if (this.clientName === EModelEndpoint.agents) {
      ...
      if (editedIndices.length > 0) {
        logger.debug('[BaseClient] 已截断工具调用输出:', editedIndices);
    ...
    logger.debug('[BaseClient] 上下文计数 (1/2)', {
      remainingContextTokens,
    ...
    if (diff > 0) {
      ...
      logger.debug(
        `[BaseClient] 原始负载 (${length}) 和上下文 (${context.length}) 之间的差异: ${diff}`,
      );
    }
    ...
    if (payload.length === 0 && !shouldSummarize && latestMessage) {
      ...
      const errorMessage = `{ "type": "${ErrorTypes.INPUT_LENGTH}", "info": "${info}" }`;
      logger.warn(`提示词 token 计数超过最大 token 计数 (${info})。`);
      throw new Error(errorMessage);
    } else if (
      _instructions &&
      payload.length === 1 &&
      ...
      const errorMessage = `{ "type": "${ErrorTypes.INPUT_LENGTH}", "info": "${info}" }`;
      logger.warn(
        `包含指令后，提示词 token 计数超过剩余最大 token 计数 (${info})。`,
      );
      throw new Error(errorMessage);
    }
    ...
    logger.debug('[BaseClient] 上下文计数 (2/2)', {
      remainingContextTokens,
      maxContextTokens: this.maxContextTokens,
    });
    ...
    logger.debug('[BaseClient] token 计数映射:', tokenCountMap);
  ...
  async sendMessage(message, opts = {}) {
    ...
    this.sendMessageStartTime = Date.now();
    ...
    if (tokenCountMap) {
      ...
      if (tokenCountMap[userMessage.messageId]) {
        ...
        logger.debug('[BaseClient] 用户消息', userMessage);
      }
    ...
    if (this.options.attachments) {
      try {
        ...
        logger.error('[BaseClient] 映射对话附件时出错', error);
      }
    }
    ...
    logger.info({
      ...
      totalDurationMs: Date.now() - (this.sendMessageStartTime || Date.now()),
    });
  ...
  async loadHistory(conversationId, parentMessageId = null) {
    logger.debug('[BaseClient] 正在加载历史记录:', { conversationId, parentMessageId });
