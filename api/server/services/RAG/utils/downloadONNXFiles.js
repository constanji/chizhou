/**
 * 下载 ONNX 模型所需的所有文件
 * 使用方法: node api/server/services/RAG/utils/downloadONNXFiles.js
 */
...
const resourcesDir = path.join(__dirname, '../onnx/embedding/resources');
const baseUrl = 'https://huggingface.co/Xenova/bge-small-zh-v1.5/raw/main';

// 需要下载的文件列表
const filesToDownload = [
  {
    url: `${baseUrl}/tokenizer.json`,
    filename: 'bge-small-zh-v1.5-q-tokenizer.json',
...
async function downloadAllFiles() {
  ...
  if (allRequired) {
    console.log('\n✅ 所有必需文件已就绪！');
    console.log('现在可以运行测试: node api/server/services/RAG/utils/testONNXEmbedding.js');
  } else {
    console.log('\n❌ 部分必需文件缺失，请检查上面的错误信息');
  }
}
