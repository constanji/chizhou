...
async function flushRedisCache(dryRun = false, verbose = false) {
  try {
    ...
    if (dryRun) {
      ...
      try {
        ...
        if (useCluster) {
          ...
          console.log(`   检测到集群: ${nodes.length} 个主节点`);
        ...
        console.log(`   将删除 ${allKeys.length} 个键`);
    ...
    if (useCluster) {
      ...
      console.log(`✅ Redis 集群缓存清理成功 (${nodes.length} 个主节点)`);
    } else {
      await redis.flushdb();
...
async function main() {
  ...
  if (success) {
    if (dryRun) {
      ...
      console.log('⚠️  注意: 所有用户都需要重新进行身份验证');
    }
