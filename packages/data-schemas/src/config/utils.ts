 ...
 * Priority:
 * 1. AIPYQ_LOG_DIR environment variable
 * 2. If running within Aipyq monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  ...
  // For Aipyq project structure, use api/logs
  // For external consumers, they should set AIPYQ_LOG_DIR
  if (cwd.includes('Aipyq')) {
    return apiLogsPath;
  }
