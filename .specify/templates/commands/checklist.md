...
**比喻**：如果你的规范是用英语编写的代码，那么检查清单就是它的单元测试套件。你正在测试需求是否写得好、完整、明确并准备好实施 - 而不是实施是否有效。
...
2. **澄清意图（动态）**：推导最多三个初始上下文澄清问题（无预构建目录）。它们必须：
   ...
   生成算法：
   1. 提取信号：功能领域关键字（例如，auth、latency、UX、API）、风险指标（"critical"、"must"、"compliance"）、利益相关者提示（"QA"、"review"、"security team"）和明确的交付物（"a11y"、"rollback"、"contracts"）。
   2. 将信号聚类到候选焦点区域（最多 4 个），按相关性排序。
   ...
   5. 从这些原型中选择问题：
      ...
      - 受众框架（例如，"这仅由作者使用还是在 PR 审查期间由同行使用？"）
      - 边界排除（例如，"我们是否应该明确排除这一轮的性能调整项目？"）
   ...
   输出问题（标记 Q1/Q2/Q3）。回答后：如果 ≥2 个场景类别（替代/异常/恢复/非功能领域）仍然不清楚，你可以询问最多两个更有针对性的后续问题（Q4/Q5），每个问题一行理由（例如，"未解决的恢复路径风险"）。不要超过五个总问题。如果用户明确拒绝更多，则跳过升级。
...
5. **生成检查清单** - 创建"需求的单元测试"：
   ...
   - 生成唯一的检查清单文件名：
     ...
     - 如果文件存在，追加到现有文件
   - 从 CHK001 开始按顺序编号项目
   - 每次运行 `/speckit.checklist` 都会创建一个新文件（不会覆盖现有检查清单）
   ...
   **按质量维度的示例**：
   ...
   - "'突出'是否用可衡量的视觉属性定义？[模糊性, Spec §FR-4]"
   ...
   **场景分类和覆盖**（需求质量焦点）：
   ...
   - 如果场景类别缺失："[场景类型] 需求是否被有意排除或缺失？[Gap]"
   ...
   **发现和解决问题**（需求质量问题）：
   询问需求本身的问题：
   - 模糊性："'快速'一词是否用具体指标量化？[模糊性, Spec §NFR-1]"
   ...
   **🚫 绝对禁止** - 这些使其成为实施测试，而非需求测试：
   - ❌ 任何以 "Verify"、"Test"、"Confirm"、"Check" + 实施行为开头的项目
   - ❌ 对代码执行、用户操作、系统行为的引用
   - ❌ "正确显示"、"正常工作"、"按预期运行"
   - ❌ "点击"、"导航"、"渲染"、"加载"、"执行"
   - ❌ 测试用例、测试计划、QA 程序
...
6. **结构参考**：按照 `templates/checklist-template.md` 中的规范模板生成检查清单，用于标题、元部分、类别标题和 ID 格式化。如果模板不可用，使用：H1 标题、目的/创建元行、包含 `- [ ] CHK### <requirement item>` 行的 `##` 类别部分，全局递增 ID 从 CHK001 开始。
...
**重要提示**：每次调用 `/speckit.checklist` 命令都会使用简短、描述性的名称创建检查清单文件（除非文件已存在）。这允许：
