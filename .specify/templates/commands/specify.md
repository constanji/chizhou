...
用户在触发消息中 `/speckit.specify` 之后输入的文字**就是**功能描述。假设你在此对话中始终可以获取它，即使下面字面显示了 `{ARGS}`。除非用户提供了空命令，否则不要要求用户重复。
...
2. **在创建新分支之前检查现有分支**：
   ...
   **重要**：
   ...
   - 每个功能只能运行此脚本一次
   - JSON 在终端中作为输出提供 - 始终引用它以获取你正在寻找的实际内容
...
6. **规范质量验证**：在编写初始规范后，根据质量标准对其进行验证：
   ...
   c. **处理验证结果**：
      ...
      - **如果 [需要澄清] 标记仍然存在**：
        1. 从规范中提取所有 [需要澄清：...] 标记
        2. **限制检查**：如果存在超过 3 个标记，仅保留 3 个最关键的（按范围/安全/UX 影响），其余做出明智的猜测
...
7. 报告完成情况，包括分支名称、规范文件路径、检查清单结果，以及下一阶段的准备情况（`/speckit.clarify` 或 `/speckit.plan`）。
...
5. **像测试人员一样思考**：每个模糊的需求都应该在"可测试且明确"的检查清单项目中失败
